...
public class MainMenuUIManager : MonoBehaviour
{
    ...
    void RefreshUI()
    {
        ...
        {
            var s = sessions[i];
            string dateStr = (s.last_date != null) ? s.last_date : "Пусто";
    ...
    void OnSettingsClicked()
    {
        ...
        {
            Debug.Log("Загрузка сцены настроек...");

            // Закрываем БД перед сменой сцены (как в других методах)
            if (db != null)
                db.Close();
        ...
        {
            Debug.Log("Сначала выберите сессию!");
    ...
    void OnStartSelected()
    {
        ...
        // Демонстрация работы с уровнями
        // уровень 5 (0-9)
        int LevelNum = 5;
        Task[] Tasks = db.Table<Task>().OrderBy(t => t.level_id).ToArray();
        Debug.Log("Task: " + Tasks[LevelNum].name);
        Debug.Log("Answer: " + Tasks[LevelNum].answer);
...
#if UNITY_EDITOR
        // Режим редактора - останавливаем проигрывание
        UnityEditor.EditorApplication.isPlaying = false;
#else
        // Собранная версия - закрываем приложение
        Application.Quit();
#endif
    }
    ...
    void InitDbIfNeeded()
    {
        ...
        if (IsNeedToUpdate)
        {
            ...
            Debug.Log("Настройки по умолчанию созданы");
